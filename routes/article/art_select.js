...
router.post('/', async(ctx, next) => {
    ...
    if (id) {
        ...
        var sql = "SELECT hits.views,news.* FROM (SELECT SUBSTRING_INDEX(hitsid, '-', -1) AS hid,views FROM v9_hits) AS hits LEFT JOIN v9_news AS news ON hits.hid=news.id ORDER BY id desc limit " + count + "," + pagesize;
    }
    console.log(sql)
